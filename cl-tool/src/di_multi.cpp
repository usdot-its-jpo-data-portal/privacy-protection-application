...
namespace DIMulti {
    ...
    trajectory::Trajectory DICSV::DeIdentify(trajectory::Trajectory& traj, const std::string& uid, std::shared_ptr<instrument::PointCounter> pc) const {

        std::string shape_in_file_path, shape_out_file_path;
    
        ErrorCorrector ec(50, pc );
    ...
    void DICSV::Thread(unsigned thread_num, MultiThread::SharedQueue<FileInfo::Ptr>* q) {
        ...
        // polymophism through shared pointers.
        std::shared_ptr<trajectory::TrajectoryWriter> tw  = nullptr;
        std::shared_ptr<trajectory::TrajectoryFactory> tr = nullptr;
        std::shared_ptr<instrument::PointCounter> pc      = nullptr;

        if ( count_points_ ) {
            // each thread has its own counter; a total counter across all trips.
            pc = counters_[thread_num];
        }

        // The only state the reader and writer saves is the UID.  That is recomputed per make_trajectory call.
        switch ( config_ptr_->tracktype ) {
            case Config::TRACKTYPE::SHRP2 :
                tr = std::make_shared<track_types::SHRP2Reader>( *config_ptr_, pc );
                tw = std::make_shared<track_types::SHRP2Writer>( out_dir_path_, *config_ptr_ );
                break;
            case Config::TRACKTYPE::BSMP1 :
            default :
                tr = std::make_shared<track_types::BSMP1CSVTrajectoryFactory>( pc );
                tw = std::make_shared<track_types::BSMP1CSVTrajectoryWriter>( out_dir_path_ );
        }

        while ((trip_file_ptr = std::dynamic_pointer_cast<SingleFileInfo>(q->pop())) != nullptr) {

            try {
                traj = tr->make_trajectory( trip_file_ptr->GetFilePath() );
                tw->write_trajectory( DeIdentify(traj, tr->get_uid(), pc ), tr->get_uid(), true );
